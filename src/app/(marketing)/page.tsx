  ...
  return (
    ...
    >
      <div className="absolute inset-0 overflow-hidden pointer-events-none">
        ...
        />
        <div
          className="absolute bottom-32 inset-x-0 h-px opacity-20"
      ...
      <div className="max-w-7xl mx-auto px-6 pt-24 pb-20 relative z-10">
        <div className="max-w-3xl">
          <FadeUp>
            <div
              className="inline-flex items-center mb-6 text-xs font-semibold tracking-widest uppercase border px-3 py-1 rounded-full"
    ...
    <section id="servicos" className="py-24" style={{ background: `linear-gradient(180deg, ${NAVY} 0%, ${NAVY_DEEP} 100%)` }}>
      <div className="max-w-7xl mx-auto px-6">
        ...
        <FadeUp delay={0.4} className="text-center mt-12">
          <Link
            href="#contato"
            className={cn(buttonVariants({ size: "lg" }), "font-semibold cursor-pointer")}
            style={{ background: GOLD, color: NAVY_DEEP, borderColor: "transparent" }}
          >
...
// ─── Depoimentos ─────────────────────────────────────────────────────
const depoimentos = [
  { nome: "Carolina Duarte", cargo: "Diretora de RH", empresa: "Grupo Saúde São Lucas", texto: "O workshop do André transformou como nossos gestores lidam com situações de pressão. A analogia com a aviação torna tudo concreto e aplicável. Resultado imediato." },
  { nome: "Ricardo Almeida", cargo: "CEO", empresa: "Construtora Alvorada", texto: "Já fizemos vários treinamentos ao longo dos anos. O André é diferente — ele começa entendendo quem somos antes de falar qualquer coisa. O treinamento foi feito para nós." },
  { nome: "Fernanda Costa", cargo: "Gerente de Operações", empresa: "Logtech Brasil", texto: "A sessão sobre comunicação em situações críticas mudou nossa cultura de reuniões. Hoje somos mais diretos, eficientes e muito menos reativos." },
]
      ...
      <div className="max-w-7xl mx-auto px-6">
        ...
        <div className="grid md:grid-cols-3 gap-8">
          {depoimentos.map((dep, i) => (
            <FadeUp key={dep.nome} delay={i * 0.1}>
              <div className="flex flex-col h-full p-8 rounded-xl bg-white/6 border border-white/10 hover:bg-white/10 transition-all duration-300">
                ...
                <p className="text-white/80 text-sm leading-relaxed flex-1 italic">&ldquo;{dep.texto}&rdquo;</p>
                <div className="mt-8 pt-6 border-t border-white/10">
                  <p className="font-semibold text-white text-sm">{dep.nome}</p>
                  <p className="text-white/40 text-xs mt-0.5">{dep.cargo} · {dep.empresa}</p>
                </div>
              </div>
            </FadeUp>
          ))}
...
// ─── Blog ─────────────────────────────────────────────────────────────
const postsMock = [
  { slug: "comunicacao-cockpit-para-sala-de-reuniao", tag: "Comunicação", titulo: "O que a comunicação do cockpit pode ensinar às suas reuniões", resumo: "Na aviação, comunicação ambígua custa vidas. Nas empresas, custa projetos, clientes e talentos.", tempo: "5 min" },
  { slug: "lideranca-sob-pressao-licoes-aviacao", tag: "Liderança", titulo: "Liderança sob pressão: lições de quem comanda aeronaves", resumo: "Como tomar decisões rápidas, corretas e seguras quando o tempo é curto e as consequências são altas.", tempo: "7 min" },
  { slug: "fator-humano-por-que-times-erram", tag: "Fator Humano", titulo: "Fator humano: por que times de alta performance ainda erram", resumo: "Entender por que erros acontecem em equipes competentes é o primeiro passo para uma cultura de segurança.", tempo: "6 min" },
]
            ...
            <FadeUp key={post.slug} delay={i * 0.1}>
              <Link href={`/blog/${post.slug}`} className="group block cursor-pointer">
                ...
                <div className="flex items-center gap-3 mb-3">
                  <span className="text-xs font-medium px-2 py-0.5 rounded" style={{ background: `${GOLD}15`, color: NAVY }}>{post.tag}</span>
                  <span className="text-xs text-slate-400">{post.tempo} de leitura</span>
                </div>
                <h3 className="font-bold text-lg leading-tight mb-2 group-hover:underline" style={{ color: NAVY }}>{post.titulo}</h3>
                <p className="text-slate-500 text-sm leading-relaxed line-clamp-2">{post.resumo}</p>
              </Link>
            </FadeUp>
          ))}
        </div>
        ...
        <div className="grid lg:grid-cols-2 gap-16">
          <FadeUp>
            ...
            <div className="space-y-4">
              <a
                href={`https://wa.me/5511999999999?text=Olá André, gostaria de saber mais sobre os treinamentos.`}
                target="_blank"
                rel="noopener noreferrer"
                className="flex items-center gap-4 p-5 rounded-xl border border-white/10 hover:border-green-400/40 hover:bg-green-400/5 transition-all duration-300 group cursor-pointer"
              >
